...
tokenizer = RobertaTokenizer.from_pretrained(PRE_TRAINED_MODEL_NAME)s
...
def _preprocess_file(file, balance_dataset, max_seq_length, prefix):
    """Summary

    Args:
        file (TYPE): Description
        balance_dataset (TYPE): Description
    ...
    print(
        'df[input_ids] after calling convert_to_bert_input_ids: {}'.format(
            df['input_ids']
        )
    )
...
def split_df(df, train_rate, validation_rate, balance_dataset):
...
def parse_args():
    # Unlike SageMaker training jobs (which have `SM_HOSTS` and `SM_CURRENT_HOST` env vars),
    # processing jobs to need to parse the resource config file directly
